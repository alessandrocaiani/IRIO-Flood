...
# Losses <- readRDS("../2024_11_19_00_07_47_EmiliaGDPLoss_Sensitivity.rds")
# Losses <- readRDS("EmiliaGDPLoss_Sensitivity_2024-11-21_09_27_46.rds")
Losses <- readRDS("EmiliaGDPLoss_Sensitivity_2024-11-22_15_21_11.rds")
#Probably running with the sample from the datacenter the results are different
DATACENTER=args[1]
filter_peaks=args[2]
if (DATACENTER) {
  Sample <- readRDS("../Sample_SensEmiliaFlood_datacenter_0.5.rds")
  #select columns that are not repeated
  Sample <- Sample[,c(1,2,3,5,6,25,28,29)]
}else{
  Sample <- readRDS("../Sample_SensitivityEmiliaFlood_0.5.rds")
}

#Remvoe all combinations that generate a GDP loss above 0.2
if (filter_peaks) {
  Sample=Sample[Losses[,4] < 0.2,]
  Losses=Losses[Losses[,4] < 0.2,]
}

#A check on the condition that they are the same size
if(dim(Losses)[1]!=dim(Sample)[1]) {
  print(paste0("Error: Losses and Sample have different dimensions"))
  print(dim(Losses)[1])
  print(dim(Sample)[1])
}
...
for (name_col in colnames(Losses)) {
  ...
  ggplot(gdp_summary, aes(x = last)) +
    ...
    geom_line(aes(y = median_gdp), color = "red", linewidth = 1.2) +  # Red line for median
    labs(
  ...
  gdp_summary <- data %>%
    {if(filter_peaks) filter(.,gamma3 > 6) else .}  %>% 
    group_by(gamma3) %>%
    summarize(median_gdp = median(GDPloss),
              q1_gdp = quantile(GDPloss, 0.25),
              q3_gdp = quantile(GDPloss, 0.75)) %>%
  ...
  ggplot(gdp_summary, aes(x = gamma3)) +
    ...
    {if(filter_peaks) ylim(min(gdp_summary$q1_gdp),max(gdp_summary$q3_gdp))}+
    {if(filter_peaks) xlim(min(data$gamma3)+1, max(data$gamma3))}+
    theme_bw()+
    theme(plot.title = element_text(size=22,hjust = 0.5),
          axis.text = element_text(size=12))+
  ...
  gdp_summary <- data %>%
    {if(filter_peaks) filter(.,gamma4 > 1) else .}  %>% 
    group_by(gamma4) %>%
    summarize(median_gdp = median(GDPloss),
              q1_gdp = quantile(GDPloss, 0.25),
              q3_gdp = quantile(GDPloss, 0.75)) %>%
  ...
  ggplot(gdp_summary, aes(x = gamma4)) +
    ...
    {if(filter_peaks) ylim(min(gdp_summary$q1_gdp),max(gdp_summary$q3_gdp))}+
    {if(filter_peaks) xlim(2, max(data$gamma4))}+
    theme_bw()+
    theme(plot.title = element_text(size=22,hjust = 0.5),
          axis.text = element_text(size=12))+
  ...
  gdp_summary <- data %>%
    {if(filter_peaks) filter(.,gamma5 > 1) else .}  %>% 
    group_by(gamma5) %>%
    summarize(median_gdp = median(GDPloss),
              q1_gdp = quantile(GDPloss, 0.25),
              q3_gdp = quantile(GDPloss, 0.75)) %>%
  ...
  ggplot(gdp_summary, aes(x = gamma5)) +
    ...
    {if(filter_peaks) ylim(min(gdp_summary$q1_gdp),max(gdp_summary$q3_gdp))}+
    {if(filter_peaks) xlim(2, max(data$gamma5))}+
    theme_bw()+
    theme(plot.title = element_text(size=22,hjust = 0.5),
          axis.text = element_text(size=12))+
  ...
  gdp_summary <- data %>%
    {if(filter_peaks) filter(.,gamma6 > 1) else .}  %>% 
    group_by(gamma6) %>%
    summarize(median_gdp = median(GDPloss),
              q1_gdp = quantile(GDPloss, 0.25),
              q3_gdp = quantile(GDPloss, 0.75)) %>%
  ...
  ggplot(gdp_summary, aes(x = gamma6)) +
    ...
    {if(filter_peaks) ylim(min(gdp_summary$q1_gdp),max(gdp_summary$q3_gdp))}+
    {if(filter_peaks) xlim(2, max(data$gamma6))}+
    theme_bw()+
    theme(plot.title = element_text(size=22,hjust = 0.5),
          axis.text = element_text(size=12))+
    theme(plot.margin = unit(c(1, 1, 1, 1), "cm")) -> gamma_6  # Increase the margin (top, right, bottom, left
  
  list_plot[[name_col]] = list(alpha,beta,gamma_1,gamma_2,gamma_3,gamma_4,gamma_5,gamma_6)

  
  # library(patchwork) 
  # #Assemble the plots into a panel
  # (alpha | beta ) /
  # (gamma_1 | gamma_2 ) /
  # (gamma_3 | gamma_4 ) /
  # (gamma_5 | gamma_6 ) 
  ...
  # if(DATACENTER){
  #   if(filter_peaks){
  #     ggsave(paste0("panel_sensitivity_",name_col,"_datacenter_range_50_peaks.jpeg"), width = 20, height = 40, units = "cm")
  #   }else{
  #     ggsave(paste0("panel_sensitivity_",name_col,"_datacenter_range_50.jpeg"), width = 20, height = 40, units = "cm")
  #   }
  # }else{
...
jpeg(paste0("../results/panel_sensitivity_",names(list_plot)[1],".jpeg"),width = 20,height = 35,units = "cm",res=300)
ggpubr::ggarrange(
  plotlist = list_plot[[1]],
  nrow=4,ncol=2
)  # 
dev.off()

jpeg(paste0("../results/panel_sensitivity_",names(list_plot)[2],".jpeg"),width = 20,height = 35,units = "cm",res=300)
ggpubr::ggarrange(
  plotlist = list_plot[[2]],
  nrow=4,ncol=2
)  # 

dev.off()
jpeg(paste0("../results/panel_sensitivity_",names(list_plot)[3],".jpeg"),width = 20,height = 35,units = "cm",res=300)
ggpubr::ggarrange(
  plotlist = list_plot[[3]],
  nrow=4,ncol=2
)  # 
dev.off()

jpeg(paste0("../results/panel_sensitivity_",names(list_plot)[4],".jpeg"),width = 20,height = 35,units = "cm",res=300)
ggpubr::ggarrange(
  plotlist = list_plot[[4]],
  nrow=4,ncol=2
)  # 
dev.off()
